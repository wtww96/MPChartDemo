...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected float textHeight;//文字高度

    public BarChartRenderer(BarDataProvider chart, ChartAnimator animator,
                            ViewPortHandler viewPortHandler) {
        super(animator, viewPortHandler);
        this.mChart = chart;
        textHeight = Utils.calcTextHeight(mValuePaint, "8");
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        if (mChart.isDrawBarShadowEnabled()) {
            ...
            for (int i = 0, count = Math.min((int) (Math.ceil((float) (dataSet.getEntryCount()) * phaseX)),
                ...
                if (mChart.isDrawBarRoundedRadius() > 0) {
                    //修改为绘制圆角
                    c.drawRoundRect(mBarShadowRectBuffer, mChart.isDrawBarRoundedRadius(), mChart.isDrawBarRoundedRadius()
                            , mShadowPaint);
                } else {
            ...
            if (isCustomFill) {
                ...
                if (mChart.isDrawBarRoundedRadius() > 0) {
                    ...
                    c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                            buffer.buffer[j + 3], mChart.isDrawBarRoundedRadius(), mChart.isDrawBarRoundedRadius(),
                            mRenderPaint);
                } else {
    ...
    public void drawValue(Canvas c, String valueText, float x, float y, int color) {
        ...
        c.drawText(valueText, x, textHeight + dp2px(20), mValuePaint);
    }
