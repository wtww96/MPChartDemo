...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public void drawData(Canvas c) {
        ...
        //todo 修改线性图值为0的时候 画的线被遮挡一部分 -2.5
        c.drawBitmap(drawBitmap, 0, -2.5f, mRenderPaint);
    }
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                int y = 0;
                float y1 = prev.getY() * phaseY + y;
                float y2 = cur.getY() * phaseY + y;
                float y3 = cur.getY() * phaseY + y;
                //cubicPath.cubicTo(
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            // That's because we need 4 points for a cubic bezier (cubic=4), otherwise we get lines moving and doing weird stuff on the edges of
            // the chart.
